...
if __name__ == '__main__':  # IMPORTANT FOR PARALLEL PROCESSES
    ...
    if RealData is True:  # realdata
        exp = experiment.Chiaro(
            '/Users/giuseppeciccone/OneDrive - University of Glasgow/PhD/Nanoindentation/Data/PDMS/2020_12_7_PDMS_bilayers_stiffprobe/10_1_homogeneous/10_1_homogeneous/matrix_scan01/')
        exp.browse()
        exp[0].open()
        cv = exp[0][1]
        x = cv.z
    ...
    names = ['iterative finite diff',
             'smooth finite diff (mean)', 'smooth finite diff (mean) opt']
    derivates = []
    ...
    x_hat, dxdt_hat = pynumdiff.finite_difference.first_order(
        y, dx, params=[1000], options={'iterate': True})
    ...
    x_hat2, dxdt_hat2 = pynumdiff.finite_difference.first_order(
        y, dx, params, options={'iterate': True})
    ...
    for i in range(len(derivates)):
        ...
        plt.plot(x[1:], derivates[i][1:], label=names[i], alpha=0.8)
    plt.legend()
