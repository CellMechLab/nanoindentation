...
GenerateFake_params = [ 1, 4000,  20000, 2000, 300, 0] #noiselevel, maxlength, E1, E2, d0, mode: 0=50*same curve, 1=curves for different d
...
for cut in cuts:
    cutoff=cut
    Elasto3_params = [30, 500, 2000, cutoff]
    c.b3_Alistography(params=Elasto3_params)
    d0s1.append(c.pars1[2])
    d0s2.append(c.pars2[2])
print('Step 7: Elastic spectra calculated!')
print(cuts)
print(d0s1)
print(d0s2)
y=np.asarray(cuts)*np.asarray(d0s1)/np.asarray(d0s2)
plt.plot(cuts,y)
plt.ylim(0.95, 1.05)
