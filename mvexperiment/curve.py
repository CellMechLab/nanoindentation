...
class Segment(object):
    def __init__(self, parent=None, z = None, f = None):
        ...
        self.mode_feedback = None   #indicates whether the feedback is active (for pause)
        ...
        self.youngIThreshold = None #Last point of the indentation used to make the Hertzian fit
        self.indentation = None     #indentation
        self.touch = None           #force corresponding to indentation, only to simplify plotting and managing
        ...
        self._filterLength = 1 / 30 #for the general smoothing (smoothCurve function)
        self._contactLength = 1/20  #There should be at leat this fraction of the curve before the contact point
    ...
    def plot(self,mode = 'curve',label=None, color='b',alpha=1):
        if mode == 'curve':
            ...
            if self.outContact > 0:
                plt.plot(self.z[:self.outContact], self.f[:self.outContact], 'y-')
            if self.iContact > 0:
                plt.plot([self.z[self.iContact]], [self.f[self.iContact]], 'go')
            plt.ylabel('Force [nN]')
            plt.legend()
        elif mode=='light':
            if self.iContact > 0:
                ...
                plt.plot(self.z-offsetX, self.f-offsetY, color=color,ls= '-',alpha=alpha)
            else:
                plt.plot(self.z, self.f, color=color,ls= '-',alpha=alpha)
        elif mode == 'indlight':
            if self.indentation is not None:
                plt.plot(self.indentation, self.touch, color=color,ls= '-',alpha=alpha)
            ...
            if self.indentation is not None:
                if label is None:
                    ...
                    plt.plot(self.indentation, self.touch, color=color,ls= '-',label=label)
                if self.young is not None:
                    if self.youngIThreshold is not None:
                        plt.plot(self.indentation[:self.youngIThreshold], self.hertz(self.indentation[:self.youngIThreshold]), 'g-',label='Hertz fit E={}'.format(int(self.young*1e9)))
                    else:
                        plt.plot(self.indentation,self.hertz(self.indentation),'g-',label='Hertz fit E={}'.format(int(self.young*1e9)))
            plt.ylabel('Force [nN]')
            plt.legend()
            ...
            if self.elastography is not None and self.elastography.Ex is not None:
                plt.xlabel('Indentation [nm]')
                if label is None:
                    plt.plot(self.elastography.Ex, self.elastography.Ey, color=color, ls='-')
                else:
                    plt.plot(self.elastography.Ex, self.elastography.Ey, color=color, ls='-', label=label)
                if self.elastography.bilData is not None:
                    ec,eb,t = self.elastography.bilData
                    lbl = 'Bilayer fit\nE_c={}\nE_b={}\bT={}'.format(int(ec * 100)/100,int(eb*100)/100,int(t))
                    plt.plot(self.elastography.Ex, self.elastography.EBilayer(self.elastography.Ex,ec,eb,t) , 'g-', label=lbl)
    ...
    def hertz(self, x, E=None): #NB E should be in nN/nm^2 = 10^9 N/m^2 -> internal units for E is GPa
        ...
        y = (4.0 / 3.0) * (E / (1 - self.poisson ** 2)) * np.sqrt(self.parent.tip_radius * x ** 3)
