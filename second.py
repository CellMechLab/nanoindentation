...
exp = experiment.Chiaro(
    '/Users/giuseppeciccone/OneDrive - University of Glasgow/PhD/Nanoindentation/Data/PDMS/2020_12_3_PDMS_bilayers/ind1/10_1 homogeneous/10_1 homogeneous/matrix_scan01')
exp.browse()
...
for c in tqdm(exp.haystack):
    ...
    c[1].Ecap = ES_prime(c[1], cp=c[1].cp, k=c.cantilever_k,
                         R=c.tip_radius*1e-9, capped=False)
    plt.plot(c[1].Ecap[0], c[1].Ecap[1])
plt.subplot(212)
for c in tqdm(exp.haystack):
    #c[1].E = ES_prime(c[1],cp = c[1].cp,k=c.cantilever_k,R=c.tip_radius*1e-9)
    c[1].Ecap = ES_direct(
        c[1], cp=c[1].cp, k=c.cantilever_k, R=c.tip_radius*1e-9)
    plt.plot(c[1].Ecap[0], c[1].Ecap[1])
